 ...
 *
 * FIX: Uses pi.sendUserMessage() instead of pi.sendMessage({ customType: "answers" })
 * to ensure the answers are displayed correctly in the TUI (no registerMessageRenderer needed).
 */
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			// Use the current model for extraction (no point trying unavailable providers)
			...
			// FIX: Use sendUserMessage instead of sendMessage with customType "answers"
			// sendUserMessage displays correctly in the TUI as a real user message
			// and automatically triggers a turn so the assistant can respond.
			pi.sendUserMessage(
